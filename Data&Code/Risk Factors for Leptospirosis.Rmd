...
glm_selected <- glm(result ~ gender + hhoccup + landuse, 
                  data = lep_narm,
                  family = binomial(link = "logit"))
...
drop1(glm_selected, test = "Chisq")
```


```{r}
# glmer with random effect of constituency on intercept
glmer_c_1 <- glmer(result ~ gender  + landuse + hhoccup + (1 | constituency), 
                   data = lep_narm,
                   family = binomial(link = "logit"))

# ANOVA
anova(glmer_c_1, glm_selected)
```
```{r}
# constituency model considering random slopes
# gender
glmer_c_2 <- glmer(result ~ gender + landuse + hhoccup + 
                     (1 + gender| constituency), 
                 data = lep_narm,
                 family = binomial(link = "logit"))

# landuse
glmer_c_3 <- glmer(result ~ gender + landuse + hhoccup + 
                     (1 + landuse| constituency), 
                 data = lep_narm,
                 family = binomial(link = "logit"))

# hhoccup
glmer_c_4 <- glmer(result ~ gender + landuse + hhoccup + 
                     (1 + hhoccup| constituency), 
                 data = lep_narm,
                 family = binomial(link = "logit"))

# ANOVA
anova(glmer_c_1, glmer_c_2, glmer_c_3, glmer_c_4)
```
...
# glmer with random effect of village on intercept
glmer_v <- glmer(result ~ gender + landuse + hhoccup + (1 | village),  
                 data = lep_narm, 
                 family = binomial(link = "logit"))

# AIC
AIC(glmer_c_1, glmer_l, glmer_v)
```

```{r}
# constituency model summary
summary(glmer_c_1)
```
...
sim <- simulateResiduals(glmer_c_1)
plot(sim)
```
...
plot(sim, form = lep_narm$altitude)
