...
```{r}
# an initial model
model_1 <- glmer(result ~ 1 + gender + age + occupation * landuse + altitude +
...
```
```{r}
model_2 <- glm(result ~ 1 + gender + age + occupation * landuse + altitude +
                   hhoccup + disthosp, data = lep_narm,
                 family = binomial(link = "logit"))
