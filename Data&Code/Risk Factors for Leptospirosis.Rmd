...
p1 <- ggplot(data = lep_dat %>% 
               ...
               drop_na(occupation), 
             aes(x = factor(occupation)))+
  geom_bar(stat = "count", width = 0.45, color="darkblue", fill="lightblue") +
  labs(x ="irrigation, Tana North")
...
# identify which columns are significant
drop1(glm, test = "Chisq")
```

```{r}
# GLM with selected variables
glm_selected <- glm(result ~ gender + age + occupation * landuse + hhoccup , 
                  data = lep_narm,
                  family = binomial(link = "logit"))
...
glmer_village_1 <- glmer(result ~ gender + age + occupation * landuse + 
                           hhoccup + (1 | village), 
                         data = lep_narm,
                         family = binomial(link = "logit"))

# Wald test
anova(glmer_village_1, glm_selected)

```
```{r, warning = FALSE}
glmer_village_2 <- glmer(result ~ gender + age + occupation * landuse + 
                           hhoccup + (1 + occupation| village), 
                 data = lep_narm,
                 family = binomial(link = "logit"))
anova(glmer_village_1, glmer_village_2)
```

```{r, warning = FALSE}
glmer_location <- glmer(result ~ gender + age + occupation * landuse + 
                          hhoccup + (1 | location), 
                 data = lep_narm,
                 family = binomial(link = "logit"))

glmer_constituency <- glmer(result ~ gender + age + occupation * landuse + 
                              hhoccup + (1 | constituency),  
                            data = lep_narm, 
                            family = binomial(link = "logit"))

anova(glmer_village_1, glmer_location, glmer_constituency)
```

```{r}
summary(glmer_village_1)
```

```{r}
summary(glmer_location)
```
```{r}
summary(glmer_constituency)
```

```{r, warning = FALSE}
vif(glmer_village_1)
vif(glmer_location)
vif(glmer_constituency)
```


```{r, fig.width = 8}
# use plot function in DHARMa package for model diagnosis
sim <- simulateResiduals(glmer_constituency)
plot(sim)
```
```{r, fig.width = 8}
sim <- simulateResiduals(glmer_village_1)
plot(sim)
```
