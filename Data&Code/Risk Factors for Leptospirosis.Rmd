...
p1 <- ggplot(data = lep_dat %>% 
               filter(age >= 18 & landuse == "pastoral" & gender == "Male") %>% 
               drop_na(occupation), 
             aes(x = factor(occupation)))+
  geom_bar(stat = "count", width = 0.3, color="darkblue", fill="lightblue") +
...
p2 <- ggplot(data = lep_dat %>% 
               filter(age >= 18 & landuse == "pastoral" & gender == "Female") %>%
               drop_na(occupation), 
             aes(x = factor(occupation)))+
...
```{r}
# imputation: riverine / irrigation
idx <- which(lep_dat$age >= 18 & 
               lep_dat$landuse %in% c("riverine", "irrigation") & 
