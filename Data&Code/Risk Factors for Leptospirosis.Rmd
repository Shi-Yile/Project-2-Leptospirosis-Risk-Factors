...
lep_narm <- lep_dat %>%
  ...
  select(c(result, gender, occupation, age, landuse, hhoccup, famsize,
           disthosp, village, location, constituency))

lep_narm$result <- ifelse(lep_narm$result == "Negative", 0, 1)
# lep_narm$gender <- ifelse(lep_narm$gender == "Male", 0, 1)
...
# lep_narm$occupation <- lep_narm
```

```{r}
# an initial model
model_1 <- glmer(result ~ 1 + gender + occupation + age + landuse + hhoccup + 
                   disthosp + (1 | village), data = lep_narm,
